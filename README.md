...
4students-homepage/
│
├── index.html                    # 메인 페이지 (로그인 전)
├── signup.html                   # 회원가입 페이지
├── complete_login.html           # 로그인 완료 후 메인 페이지
├── 4s_solution.html              # 솔루션 페이지
├── css/
│   ├── style.css                 # 메인 스타일
│   ├── complete-login.css        # 로그인 후 스타일
...
├── README.md                     # 프로젝트 문서
└── SUPABASE_MIGRATION.md         # Supabase 마이그레이션 가이드
```
...
이 프로젝트는 사용자 인증을 위해 Supabase를 사용합니다. 프로젝트를 실행하기 전에 다음 단계를 따르세요:

1. **Supabase 프로젝트 생성**
   - [Supabase](https://supabase.com)에서 새 프로젝트를 생성합니다.
   - 프로젝트 URL과 Anon Key를 복사합니다.

2. **설정 파일 수정**
   - `js/supabase-config.js` 파일을 열고 다음 정보를 입력합니다:
   ```javascript
...
3. **데이터베이스 설정**
   - 상세한 설정 방법은 [SUPABASE_MIGRATION.md](SUPABASE_MIGRATION.md) 파일을 참조하세요.
...
2. **브라우저에서 열기**
   ...
   - **주의**: 파일을 직접 브라우저에서 여는 것(file://)은 CORS 문제로 인해 Supabase가 작동하지 않을 수 있습니다.
...
- **Supabase** - 인증 및 데이터베이스 (Firebase에서 마이그레이션됨)
